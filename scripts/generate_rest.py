...
# scripts/generate_python_rest_types.py — no hardcoded membership list).
#
...
# (which affects the ResourceTree accessor order + container member order — not
# alphabetical, not derivable from any spec field) and (b) the PascalCase Types
# sub-namespace for ``pubsub`` (→ ``PubSub``, not the mechanical ``Pubsub``).
# Both are kept as small explicit tables (like ATTR_OVERRIDE); the discovery
...
# spec dir is not the canonical reference name (like ATTR_OVERRIDE for accessors).
...
# ---------------------------------------------------------------------------
# Wire-type emitter (item A/H — REAL types, not `array`).
#
# For each REST namespace, emit one PHP data class per components/schemas entry
# whose schema is an OBJECT (typed public properties carrying the snake wire key,
# no methods — the Python reference records these as method-less type
# definitions, so the enumerator surfaces the bare class name with `[]` methods).
# A named public enum (x-sdk-enum, only PhoneCallHandler in relay-rest) becomes a
# PHP 8.1 backed enum. Every OTHER schema kind — a closed/open scalar or array
# alias, a oneOf/anyOf union, a bare string enum the reference inlines — is NOT
# surfaced by the Python reference (its enumerator drops module-level scalar
# TypeAlias / inline Literal), so this emitter emits nothing for it (matching the
# reference surface exactly — verified per namespace, emit-set == oracle-set).
#
# Files land one-class-per-file (PSR-4) under
#   src/SignalWire/REST/Namespaces/Generated/Types/<Ns>/<TypeName>.php
# in namespace SignalWire\REST\Namespaces\Generated\Types\<Ns>. The <Ns> subdir
# maps 1:1 to the oracle's <ns>_types_generated module (enumerate_surface.py
# routes each Types/<Ns>/ file to signalwire.rest.namespaces.<ns>_types_generated).
# The reference emits the SAME schema name into multiple <ns>_types_generated
# modules (shared SWML-schema types AIObject/Cond/… and shared error types
# Types_StatusCodes_*); PHP mirrors that per-namespace duplication faithfully
# (each spec's components/schemas carries its own copy). The SURFACE-DIFF tool's
# gen-type leaf-name fold collapses a type declared in >1 <ns>_types_generated
# module to a single gen-type.<Leaf> symbol on BOTH the reference and the port,
# so the duplicates compare equal. Property types are the port's idiom (nullable
# scalars / ?array / mixed) and do NOT affect the surface (which records only the
# class name); they exist to keep the DTOs PHPStan-L9-clean.
# ---------------------------------------------------------------------------

# The Types sub-namespaces are DISCOVERED via discover_type_ns() (see the scan
# near the top). Spec-dir -> the PHP Types subdir + the oracle
# <ns>_types_generated leaf; relay-rest folds registry; swml-webhooks is the
# types-only spec.
...
namespace SignalWire\\REST\\Namespaces\\Generated\\Types\\{sub};
"""


# PHP forbids these keywords as CLASS/ENUM names (hard reserved). A schema whose
# sanitised leaf collides gets a ``_`` suffix in the PHP class + filename; the
...
def type_name(raw: str) -> str:
    """Sanitise a components/schemas key to a valid PHP identifier, folding every
    non-identifier rune to ``_`` — matching the Go/TS/python ref_name so the LEAF
    ...
    A leaf that collides with a hard-reserved PHP keyword gets a ``_`` suffix (the
    enumerator renames it back to the bare oracle leaf)."""
    s = re.sub(r"[^A-Za-z0-9_]", "_", raw).lstrip("_")
    if not s:
        return "Schema"
    if s[0].isdigit():
        return "Schema_" + s
    if s.lower() in PHP_TYPE_RESERVED:
        return s + "_"
    return s
...
def _resolve_type_ref(schema: dict, schemas: dict, seen: set | None = None) -> dict:
    """Follow a $ref (within the SAME spec's components.schemas) and a single-member
    allOf (the '$ref + description' idiom) to the concrete schema, so a $ref-to-scalar
    newtype (uuid/jwt) / $ref-to-enum recovers its underlying scalar type. An external
    ``.json`` $ref or an unresolvable ref returns ``{}`` (→ structured/open)."""
    ...
    if ref:
        if not ref.startswith("#/"):
            return {}  # external .json ref → open structured value
        leaf = ref.rsplit("/", 1)[-1]
        if leaf in seen:
            return {}
        seen.add(leaf)
...
def php_property_type(schema: dict, schemas: dict | None = None) -> tuple[str, str | None]:
    ...
    default, so the class is a pure data holder needing no constructor. The type
    NEVER references another generated class (avoids PSR-4 ordering / cross-namespace
    collision concerns and keeps the emitter deterministic) — the surface records
    only the class name, so property types are pure idiom:
        scalar (incl. $ref-to-scalar-newtype / $ref-to-enum) -> ?string/?int/?float/?bool
        array    -> ?array  (@var list<mixed>|null)
        object / $ref-to-object / union / unknown -> ?array (@var array<string,mixed>|null)
    """
    schemas = schemas or {}
    # Resolve a $ref / single-member allOf to its concrete schema so a scalar
...
def php_property_name(wire_key: str) -> str:
    """PHP property name for a wire key. PHP property names may be reserved words
    and may contain most identifier characters; fold anything non-identifier to
...
def emit_type_class(psdk: Path, sub: str, raw_name: str, node: dict, ns_key: str, schemas: dict) -> str:
    ...
    return TYPES_HEADER.format(desc=desc, sub=sub) + "\n" + "\n".join(lines) + "\n"


def _enum_case_name(value: str) -> str:
    """PascalCase case name for a backed-enum wire value."""
...
def emit_type_enum(sub: str, enum_name: str, values: list[str], ns_key: str, raw_name: str) -> str:
    ...
    for v in values:
        ...
        while cname in used:
            cname += "_"
        used.add(cname)
        lines.append(f"    case {cname} = {php_str(v)};")
    lines.append("}")
    desc = f"Generated REST public enum for the {ns_key!r} namespace."
    return TYPES_HEADER.format(desc=desc, sub=sub) + "\n" + "\n".join(lines) + "\n"
...
def emit_types(psdk: Path, outs: dict[str, str]) -> None:
    """Emit every <ns>_types_generated PHP data class / enum into
    ``Types/<Sub>/<TypeName>.php`` keys of ``outs`` (relative to the Generated dir)."""
    for spec_dir, sub, ns_key in discover_type_ns(psdk):
        schemas = _load_types_schemas(psdk, spec_dir)
        for raw_name, node in schemas.items():
            if not isinstance(node, dict):
                continue
            # x-sdk-enum public enum → emit a backed enum (surfaced as a class).
            ...
            if is_object_schema(node):
                php_name = type_name(raw_name)
                fn = f"Types/{sub}/{php_name}.php"
                # First-seen wins within a namespace (a name can't collide inside
...
def build_outputs(psdk: Path) -> dict[str, str]:
    ...
    # Wire types (item A/H): one PHP data class / enum per components/schemas
    ...
    if args.check:
        ...
        for fn, src in outs.items():
            ...
            if not p.is_file() or p.read_text() != src:
                stale.append(str(p))
        # Detect leftover generated files no longer emitted (e.g. a renamed/removed
        # type) so GEN-FRESH is byte-exact both ways. Scan the flat Generated dir
        # and the Types/ subtree for *.php the current run did not produce.
