...
First release cut off the cross-port parity surface, and the fleet-wide
convergence version: every SignalWire SDK port declares 3.0.0. The public API is
now generated from, and continuously verified against, the shared SignalWire wire
specification (the Python SDK is the reference oracle), so this port is
functionally at parity with the other language SDKs.

Nothing in the 3.x range was ever published — this port's published tags top out
at `v1.1.0` — so the previously-drafted unreleased 3.0.2 / 3.1.0 / 3.2.0 / 3.3.0
entries are consolidated here rather than shipped as a staggered series.
