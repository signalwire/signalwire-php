...
- ruff.toml — ruff's config for the repo's 8 hand-written Python programs under scripts/, backing the REPO-LINT/REPO-FMT gates. ruff discovers its config by walking UP from the linted files to the nearest ruff.toml/.ruff.toml/pyproject.toml, so it must sit at the repo root to apply to scripts/. The gate's built-in tool-config list already excuses the sibling python entries (pyproject.toml, requirements.txt, requirements-dev.txt, pytest.ini, .coveragerc) and every other language's linter config (.golangci.yml, .rubocop.yml, clippy.toml, rustfmt.toml, .perlcriticrc, .clang-tidy, eslint.config.mjs) — ruff.toml is the same class and is simply absent from that list (porting-sdk/scripts/root_hygiene.py:56-58). Preferred fix is to add "ruff.toml" to that list, which would retire this entry; filed as a cross-repo finding since a port lane cannot edit porting-sdk. A pyproject.toml would satisfy the gate today but would falsely declare this PHP repo a Python package. (lint-php, 2026-07-30)
