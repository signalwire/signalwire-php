...
signalwire.rest._base.SignalWireRestError.request_id: reference-oracle gap — the SIGNATURE oracle records `SignalWireRestError.request_id` (porting-sdk d7c859d, the 2026-07-27 class-B2 derived-attr ruling) but python_surface.json does NOT: griffe only sees a declared member, and the reference computes this one in __init__ from the response headers. php implements the same value and the same read-back (previously listed here under the accessor spelling `get_request_id`, now FOLDED to the reference attribute name via CLASS_METHOD_ALIASES — a rename keeps comparing). The symbol IS reference surface; it is missing from the surface oracle only, so this line is a surface-oracle blind spot, not php-invented surface. Delete when python_surface.json records it.
